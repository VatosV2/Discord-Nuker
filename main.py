...
async def delete_channels(session, guild_id, bot_token):
    headers = {
...
async def main(bot_token):
    ...
    async with aiohttp.ClientSession(headers={'Authorization': f'Bot {bot_token}'}) as session:
        deleted_count = await delete_channels(session, guild_id, bot_token)
        created_count = Channel_created
        try:
            await asyncio.gather(*tasks)
        finally:
            os.system(f"python webhook.py {bot_token} {guild_id} {Channel_delted2} {Channel_created} {message}")
